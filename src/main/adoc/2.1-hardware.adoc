...
* reading data from cache is *very slow*
...
* if the data is used for conditional branching, the CPU _predicts_ the branch,
and continues execution (even though the actual data is not yet known)
* if the branch was mis-predicted, it must "clean the mess"
* good prediction 95% of the time
...
** theoretically up to +x 2+ for pure memory access (for 2 threads HT) (practically {tilde}30%)
