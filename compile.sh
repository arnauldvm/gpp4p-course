...
build_options="$build_options -a env-build"

build_options_themed="$build_options --theme volnitsky"

set -x
asciidoc $build_options_themed -a extension=html -o "$tgtpath" "$srcpath"
asciidoc $build_options -a extension=pdf -b html -a toc2! -a toc -a numbered -o - "$srcpath" | wkhtmltopdf - "$tgtpdfpath"
